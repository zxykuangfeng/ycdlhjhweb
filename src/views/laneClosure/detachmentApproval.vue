<template>
  <div class="app-container">
    ...
    <!-- 审核对话框 -->
    <el-dialog title="审核项目" :visible.sync="reviewDialogVisible" width="70%">
...
<script>
import { sgxmsList, sgxmsAudit, worksMember } from '@/api/road';
...
export default {
  ...
  data() {
    return {
      list: [],
      total: 0,
      listQuery: { page: 1, limit: 20, name: '', company_id: null },
      userList: [],
  ...
  methods: {
    ...
    submitReview(status) {
      sgxmsAudit({ id: this.reviewForm.id,auditComment: this.reviewForm.auditComment }).then((res) => {
