<template>
  <div class="app-container">
    ...
    <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
      ...
      <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
        <div class="card-panel" @click="handleSetLineChartData('purchases')">
          ...
          <div class="card-panel-description">
            <div class="card-panel-text">总消费书券</div>
            <count-to
              :start-val="0"
    ...
    >
      ...
      <el-table-column label="小说名" min-width="120px" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.bookTitle }}</span>
        </template>
      </el-table-column>
      <el-table-column label="章节名" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{ row.chapterTitle }}</span>
        </template>
      </el-table-column>
      <el-table-column label="消费书券" align="center">
        <template slot-scope="{ row }">
...
<script>
import { dataconsumptiondata, dataconsumptionexport } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      dataconsumptiondata(this.listQuery).then(res => {
        if (res.code == 0) {
          this.list = res.data
    ...
    handleDownload() {
      ...
      dataconsumptionexport(_d).then(res => {
